...
class SuratPklGroupsController extends \BaseController {
	...
	{
		return View::make('suratpklgroups.index')->withTitle('Surat Ijin PKL (Kelompok)');
	}
	...
	{
		$validator = Validator::make($data = Input::all(), SuratPklGroup::$rules);
		if ($validator->fails())
		{
			...
			$date = new \DateTime;
			$kode='SIPG'.substr(md5(date('Y m d H i s')), 0,5);
			...
			if (!empty($_POST['mynim']) && !empty($_POST['mynama'])){
				...
				if (empty($error)){
					DB::table('surat_pkl_groups')->insertGetId(
			      		array(
			                  'kode' => $kode,
				...
				$datasrt = SuratPklGroup::where('kode',$kode)
					->with('Dataprodi')
	...
	private function exportPdf($surats)
	{
		...
		$data['datas'] = $surats;
		$pdf = PDF::loadView('srtmahasiswa.SuratPklGroup',$data)->setPaper('a4');
